...
Within that consolidated pass, include a simplicity review of the applicable candidate diff. Look for behavior-preserving simplifications such as pass-through helpers, literal assembly, obsolete workarounds, duplicated guards, unnecessary copies, index-coupled parallel arrays, redundant annotations, and abstractions that do not hide meaningful complexity. Treat an introduced indirection as a blocker only when the simpler implementation preserves the finite contract and repository instructions; do not request unrelated cleanup or architectural expansion.
